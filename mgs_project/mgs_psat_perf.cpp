...
const double PATIENCE=600000; 
vector < chrono::duration<double, std::milli> > telapsed;
chrono::high_resolution_clock::time_point dt, dtap, t1, t2;
std::chrono::duration<double, std::milli> fp_ms;

vector<long> stepInCount;
...
int main(int argc, char *argv[]){
    ...
    printf("NumThreads = %d\n\n", threadCount);
    stepInCount.resize(threadCount, 0);
    ...
    for(int trial=0; trial<32; trial++){
        ...
        gate_possibilities.clear();
        TEST_COMBINATION.clear();
        ...
        if(mystery_gate_count){
            ...
            IS_SEQ, DEBUG, ckt, mystery_gate_count, gate_names, CHECK_FF, cout, gate_possibilities, quitFlag)
            {
            ...
            if(fp_ms.count() >= PATIENCE) break;

            if(solved){
                cout<<"\n\nSOLVED! in " << fp_ms.count() <<" ms\n";
                telapsed.push_back(fp_ms);
    ...
    printf("\n\n");
    cout<<fileName<<endl;
    printf("#MGATE, TIME(ms)\n");
    for(int i=0; i<telapsed.size(); i++){
        if(telapsed[i].count()<PATIENCE)
            cout<< (i+1) <<", "<< telapsed[i].count() << endl;
    }
...
void logicSim_SAT(int test_index, const eventSimulator& sim) {
    ...
    #pragma omp critical
    {
        dt = chrono::high_resolution_clock::now();
        fp_ms =  dt - t1;
        if(fp_ms.count() > PATIENCE) quitFlag++;
        fp_ms = dt - dtap;
        if(fp_ms.count() > 10000) {
            cout<<".";
            cout.flush();
            dtap = dt;
        }
